...
public class BoardTests {
    ...
    public void testPlaceShip() {
        ...
        Position position1 = new Position(0, 6, Position.Orientation.HORIZONTAL);
        ...
        assertFalse("Cannot place a boat which is already on the board",
                board.placeShip(ship1, new Position(1, 0, Position.Orientation.HORIZONTAL)));
        ...
        assertFalse("Ship must not extend off the edge of the board",
                board.placeShip(ship2, new Position(7, 0, Position.Orientation.HORIZONTAL)));
        assertFalse("Ship must not extend off the edge of the board",
                board.placeShip(ship2, new Position(0, 7, Position.Orientation.VERTICAL)));

        assertFalse("Ship cannot be placed on top of another ship",
                board.placeShip(ship2, new Position(3, 6, Position.Orientation.VERTICAL)));

        for (int i = 0; i < MainShip.LENGTH; i++) {
            assertEquals("Boat is not located at the required position(" + (i) + ", 6)",
                    ship1, board.getShip(i, 6));
        }
    ...
    public void testGetShipPosition() {
        ...
        Board board = new Board(boardWidth, boardHeight);
        Ship ship1 = new MainShip();
        Position position1 = new Position(2, 2, Position.Orientation.HORIZONTAL);
        ...
        assertTrue("(x,y) coordinate did not match", 2 == position2.x && 2 == position2.y);
    }
    ...
    public void testRemoveShip() {
        ...
        assertNull("Ship should no longer exist", board.getShip(2, 2));
        assertNull("Ship should no longer exist", board.getShip(ship1.getID()));

        board.placeShip(ship1, position1);
        Ship removedShip2 = board.removeShip(6, 2);
    ...
    public void testMoveShip() {
        ...
        board.moveShip(ship.getID(), new Position(3, 3, Position.Orientation.VERTICAL));

        assertNull("Ship not removed from prior position", board.getShip(4, 5));
        assertNull("Ship not removed from prior position", board.getShip(5, 5));
        assertNull("Ship not removed from prior position", board.getShip(7, 5));
    ...
    public void testShipCount() {
        ...
        board.placeShip(new PilotShip(), new Position(4, 5, Position.Orientation.HORIZONTAL));
        board.placeShip(new PilotShip(), new Position(4, 3, Position.Orientation.VERTICAL));
        board.removeShip(4, 5);
        board.placeShip(new PilotShip(), new Position(3, 5, Position.Orientation.HORIZONTAL));
        board.placeShip(new PilotShip(), new Position(2, 5, Position.Orientation.HORIZONTAL));
